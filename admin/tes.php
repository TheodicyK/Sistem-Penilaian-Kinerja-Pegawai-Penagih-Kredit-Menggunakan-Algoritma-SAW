...
function tentukanKategori($nilai_setoran) {
    if ($nilai_setoran > 2300000) {
        return 3;
    } elseif ($nilai_setoran >= 700000 && $nilai_setoran <= 2300000) {
        return 2;
    } else {
        return 1;
    }
...
function konversiKeterangan($keterangan) {
    // Jika ada data dalam keterangan, maka kriteria keterangan mendapatkan nilai 1
    if (!empty($keterangan)) {
        return 2;
    } else {
        return 0;
...
function konversiFoto($foto) {
    // Jika ada data dalam kolom foto, maka kriteria foto mendapatkan nilai 2
    if (!empty($foto)) {
        return 2;
    } else {
        return 0;
...
// Kriteria dan bobot
$kriteria = array('nilai_setoran' => 0.5, 'keterangan' => 0.5, 'nama_petugas' => 0.5, 'foto' => 0.5); // Sesuaikan dengan kriteria yang Anda miliki
$total_bobot = array_sum($kriteria);

// Periksa apakah semua kriteria adalah numerik
foreach ($kriteria as $kriteria_key => $bobot) {
    if (!is_numeric($bobot)) {
        // Tindakan apa yang akan diambil jika nilai bobot kriteria bukan numerik
...
foreach ($rows as $row) {
    ...
    // Menentukan kategori nilai setoran
    $kategori_setoran = tentukanKategori($nilai_setoran);

    // Menghitung nilai keterangan dan foto
    $nilai_keterangan = konversiKeterangan($keterangan);
    $nilai_foto = konversiFoto($foto);

    // Menghitung bobot berdasarkan kategori setoran
    $bobot = ($kategori_setoran == 1) ? 1 : (($kategori_setoran == 2) ? 2 : 3);

    // Menghitung perhitungan SAW
    $perhitungan_saw = $bobot * (($kategori_setoran * $kriteria['nilai_setoran']) + ($nilai_keterangan * $kriteria['keterangan']) + ($kriteria['nama_petugas']) + ($nilai_foto * $kriteria['foto']));

    // Menambahkan nilai akhir ke array nilai_akhir
    if (!isset($nilai_akhir[$nama_petugas])) {
        $nilai_akhir[$nama_petugas] = $perhitungan_saw * 0.5;
    } else {
        $nilai_akhir[$nama_petugas] += $perhitungan_saw * 0.5;
    }
}
...
// Urutkan nilai akhir dari yang tertinggi ke yang terendah
arsort($nilai_akhir);
?>
    ...
    <div class="container-fluid">
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="nav nav-pills">
                ...
                <li class="nav-item">
                    <a class="btn btn-outline-info me-2" aria-current="page" href="index.php">Home</a>
                </li>
                <li class="nav-item">
...
<div class="card custom-card">
    ...
    <table>
        ...
        <tbody>
            <?php $no = 1; ?>
            <?php foreach ($nilai_akhir as $nama_petugas => $nilai): ?>
                <tr>
                    <td><?php echo $no++; ?></td>
        ...
        <tbody>
            ...
            ?>
                <tr>
                    <td><?php echo $no++; ?></td>
                    <td><?php echo $nama_petugas; ?></td>
                    <td><?php echo tentukanKategori($nilai_setoran); ?></td>
                    <td><?php echo $nilai_keterangan; ?></td> <!-- Tampilkan nilai keterangan numerik -->
                    <td><?php echo $nilai_foto; ?></td> <!-- Tampilkan nilai foto numerik -->
                    <td><?php echo $perhitungan_saw; ?> = <?php echo $bobot; ?> * (<?php echo $kategori_setoran; ?> * <?php echo $kriteria['nilai_setoran']; ?>) + (<?php echo $nilai_keterangan; ?> * <?php echo $kriteria['keterangan']; ?>) + (<?php echo $kriteria['nama_petugas']; ?>) + (<?php echo $nilai_foto; ?> * <?php echo $kriteria['foto']; ?>)</td>
                </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
